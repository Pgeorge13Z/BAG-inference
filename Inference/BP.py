...
class VariableElimination(Inference):
    ...
    def _get_elimination_order(
        ...
        #print('self.variables,to_eliminate:',self.variables,to_eliminate)
    ...
    def _variable_elimination(
        ...
        print('elimination_order:',elimination_order)
...
class BeliefPropagation(Inference):
    ...
    def __init__(self, model,order=None):
        ...
        if not isinstance(model, JunctionTree):
            ...
            self.node_list=model.nodes
            if order=='random':   # 随机
                import random
                #print('random')
                elimination_order=random.sample(self.node_list,len(self.node_list))
                self.junction_tree = model.to_junction_tree(order=elimination_order,heuristic=None)
            elif order in ['MinWeight','WeightedMinFill','MinNeighbors','MinFill','H15']:

                from EliminationOrder import MinWeight,WeightedMinFill,MinNeighbors,MinFill,MWH
                ...
                elimination_order=elimination_model_options[order](model).get_elimination_order(self.node_list,show_progress=False)
                #print(order+':')
        ...
        #print('self.junction_tree.nodes:',self.junction_tree.add_cliques)
        self.clique_beliefs = {
        ...
        self.sepset={frozenset(cliques[0]).intersection(frozenset(cliques[1])):None
                     for cliques in self.junction_tree.edges}
        #print('self.sepsets:',self.sepset)

        self.factors = frozenset(
            self.junction_tree.nodes
        )
        #print('clique_beliefs:',self.clique_beliefs)

        ''' 存放factor,variable的相邻元素(对应相邻的factor->variables,variable->factors) '''
        self.variables_neighbor_factors={}
        self.factors_neighbor_variables={}

        #print('variables:',self.factors)
        #print('factors:',self.junction_tree.edges)
        for var in self.sepset:
            self.variables_neighbor_factors[tuple(var)]=[]
            for fac in list(set(self.factors)-set(self.junction_tree.add_cliques)):
                if set(var) & set(fac) == set(var):
                    self.variables_neighbor_factors[tuple(var)].append(fac)

        for fac in list(set(self.factors)-set(self.junction_tree.add_cliques)):
            self.factors_neighbor_variables[fac]=[]
            for var in self.sepset:
                if set(var) & set(fac) == set(var):
                    self.factors_neighbor_variables[fac].append(tuple(var))
        #print('self.variables_neighbor_factors:',self.variables_neighbor_factors)
        #print('self.factors_neighbor_variables:',self.factors_neighbor_variables)

        self.p_variables = {}  # 用来存储计算出的节点概率值
        self.messages={}

    def get_clique_edges(self):
        """
        Return the edges connecting cliques.
        """
        return self.junction_tree.edges
    ...
    def _update_beliefs(self, sending_clique, recieving_clique, operation):
        ...
        #print('sigma:\n',sigma)
        #print('clique_beliefs[sending_clique]:\n', self.clique_beliefs[sending_clique])
        #print('self.sepset_beliefs[sepset_key]:\n', self.sepset_beliefs[sepset_key])
        #print('-------       ---------    -------    ---------        ---')
        ...
        #self.sepset[sepset]=self.sepset_beliefs[sepset_key]
    ...
    def _calibrate_junction_tree(self, operation):
        ...
        """
        time_calibrate=time.time()

        # Initialize clique beliefs as well as sepset beliefs


        #self.factors_to_variables={}
        #self.variables_to_factors={}


        #print('clique beliefs: ',self.clique_beliefs)
        #print('sepset belief: ',self.sepset_beliefs)
        #print('self.JunctionTree.nodes:',self.junction_tree.nodes)
        #print('self.factors:',self.factors)

        #print('type:',self.junction_tree.nodes)
        '''
        clique_leafs=list(filter(lambda x:len(list(self.junction_tree.neighbors(x)))==1,self.factors))
        left_cliques=set(self.factors)-set(clique_leafs)
        import random
        clique_root=random.choice(list(left_cliques))

        print('clique leafs:',clique_leafs)
        print('clique root:',clique_root)
        '''

        for clique in self.clique_beliefs:
        ...
        time_calibrate=time.time()-time_calibrate
        return time_calibrate
        #print('self.junction.edges:',self.junction_tree.edges)
        #print('self.sepset_belief:\n',self.sepset_beliefs[frozenset((('D', 'F', 'E'), ('D', 'C', 'E')))])
    ...
    def calibrate(self):
        ...
        return self._calibrate_junction_tree(operation="marginalize")
    ...
    def bp_query(self,variables,operation="marginalize",evidence=None,printable=False,joint=True,show_progress=True):
        import time
        ...
        for var in query_variables:
            if var not in self.p_variables.keys():
                node_variables=[x for x in list(range(len(self.clique_beliefs))) if var in list(self.clique_beliefs)[x]]
                #print('node_variables:',node_variables)
                nodes_with_query_variable = min(node_variables, key=lambda x: len(list(self.clique_beliefs)[x]))

                p_variable = getattr(self.clique_beliefs[list(self.clique_beliefs)[nodes_with_query_variable]], operation)(
                    list(set(list(self.clique_beliefs)[nodes_with_query_variable]) - {var}), inplace=False)
                print('P_variable:', var, '\n', p_variable)
                self.p_variables.update({var: p_variable})
            else:
                pass
                #print('already exsists...')
                #print('P_variable:', var, '\n', self.p_variables[var])
            #query_results.append(self.p_variables[var])

        time_cost=time.time()-time_pre

        return time_cost

        #print('nodes_with_query_variable:',nodes_with_query_variable)
    ...
    def _query(
        ...
        #print('subtree.edges:',subtree.edges)
